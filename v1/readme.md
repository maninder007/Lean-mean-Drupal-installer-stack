...
Markdown# Actool – Drupal 11 Multi-Environment Stack
...
Upload your public key to /home/actool/.ssh/authorized_keys
Disable root & password login:Bashnano /etc/ssh/sshd_config
PermitRootLogin no
PasswordAuthentication no
systemctl restart ssh
Log out → log back in as actool via SSH key

How to Install

Create config:Bashcat <<'EOF' > ~/actool.env
...
Run installer:Bashcurl -O https://raw.githubusercontent.com/yourusername/actool/main/actool-installer.sh
chmod +x actool-installer.sh
sudo ./actool-installer.sh fresh
Copy ~/actool_access.txt immediately → delete after backup
Change admin password right after login
...
Add at bottom:
PHP$$   settings['trusted_host_patterns'] = ['^feesix\.com   $$', '^dev\.feesix\.com$$   ', '^stg\.feesix\.com   $$'];
...
~/actool_backups/YYYY-MM-DD_HHMMSS/<env>/ → database.sql.gz + files.tar.gz
Manual Backup
Bashfor env in dev stg prod; do
  docker compose exec -T php drush @$$   env sql-dump --gzip > ~/backup-   $$(date +%F)-$env.sql.gz
done
Restore Example (dev)
Bashgunzip -c ~/backup-2026-03-20-dev.sql.gz | docker compose exec -T db mariadb -u root -prootsecret drupal_dev
Maintenance & Operations
Weekly

Backup databases & files
Rotate backups: find ~/actool_backups -mtime +30 -exec rm -rf {} +
...
textThis is your **v1 locked in** — secure, reliable, documented.
