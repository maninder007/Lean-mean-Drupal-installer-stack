...
LOCK_FILE="/tmp/feesix-install.lock"
...
main() {
  ...
  log_info "Feezix stack ready!"
  log_info "Sites:"
  log_info "  • https://$BASE_DOMAIN           (production)"
  log_info "  • https://dev.$BASE_DOMAIN       (development)"
  log_info "  • https://stg.$BASE_DOMAIN       (staging)"
  log_info "Credentials: ~/feesix_access.txt"
  [[ "$MODE" == "rerun" && "$FORCE" == true ]] && log_info "Backups saved in ~/feesix_backups/"
}
